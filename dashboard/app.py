"""APEX-RACE-AI • Pit Wall interactive motorsport telemetry cockpit."""
...
def inject_styles() -> None:
    st.markdown(
        ...
        .apex-topbar {
            ...
            margin-bottom: 18px; padding: 18px 20px; border: 1px solid rgba(255,255,255,.09);
            border-radius: 18px;
            background: radial-gradient(circle at 82% 12%, rgba(255,53,53,.14), transparent 25%),
        ...
        .section-kicker { color:#9ca3af; text-transform:uppercase; letter-spacing:.12em; font-size:.67rem; font-weight:700; margin:8px 0 5px; }
        .section-title { font-family:'Space Grotesk', sans-serif; font-size:1.15rem; font-weight:700; margin-bottom:12px; }
        .status-chip { display:inline-flex; padding:5px 9px; border-radius:999px; border:1px solid rgba(255,255,255,.09); background:rgba(255,255,255,.03); color:#d4d4d8; font-size:.68rem; font-weight:600; }
        ...
        div[data-testid="stMetric"] { background:rgba(255,255,255,.025); border:1px solid rgba(255,255,255,.07); padding:12px 14px; border-radius:14px; }
        div[data-testid="stMetricLabel"] { font-size:.72rem; }
        div[data-testid="stMetricValue"] { font-family:'Space Grotesk', sans-serif; }
        .footer-line { border-top:1px solid rgba(255,255,255,.07); padding-top:14px; margin-top:25px; color:#71717a; font-size:.72rem; }
        </style>
        """,
...
def generate_demo_telemetry(driver: str, seed: int = 27) -> pd.DataFrame:
    ...
    rpm = np.clip(11400 + (speed - speed.mean()) * 28 + rng.normal(0, 180, points), 6200, 13200)
    gear = np.clip(np.rint((speed - 55) / 38), 0, 8)
...
def load_fastf1_telemetry(year: int, grand_prix: str, session_code: str, driver: str) -> pd.DataFrame:
    ...
    telemetry["Gear"] = pd.to_numeric(telemetry["Gear"], errors="coerce").fillna(0).clip(0, 8)
    telemetry["DRS"] = (pd.to_numeric(telemetry["DRS"], errors="coerce").fillna(0) > 0).astype(int)
...
def sidebar_controls() -> dict[str, Any]:
    ...
    st.sidebar.caption("Demo mode is fully offline. FastF1 mode loads historical session data on demand.")
    ...
    if source == "Demo telemetry":
        ...
        controls["session_code"] = st.sidebar.selectbox("Session", ["R", "Q", "FP1", "FP2", "FP3", "S", "SQ"], index=0)
...
def telemetry_chart(df: pd.DataFrame, fields: list[str], height: int) -> None:
    st.line_chart(df.set_index("Distance")[fields].copy(), height=height, use_container_width=True)
...
def main() -> None:
    ...
    if controls["source"] == "Demo telemetry":
        ...
        secondary = generate_demo_telemetry(controls["driver_2"], seed=41) if controls["compare"] else None
        source_badge = "DEMO / OFFLINE"
        source_detail = "Synthetic telemetry lab"
    else:
        try:
            df = load_fastf1_telemetry(controls["year"], controls["grand_prix"], controls["session_code"], driver)
            secondary = None
    ...
    with session_cols[3]:
        st.markdown(f'<span class="status-chip info">{len(df):,} TELEMETRY ROWS</span>', unsafe_allow_html=True)
    ...
    with tabs[1]:
        ...
        st.caption("Demo coordinates are a relative circuit shape. A future track-geometry adapter can map real position telemetry.")
    ...
    with tabs[2]:
        ...
        if secondary is None:
            ...
            st.line_chart(comparison.set_index("Distance")["Speed Delta"], height=330, use_container_width=True)
            delta_cols = st.columns(3)
            delta_cols[0].metric("MAX POSITIVE DELTA", f"{comparison['Speed Delta'].max():.1f} km/h")
            delta_cols[1].metric("MAX NEGATIVE DELTA", f"{comparison['Speed Delta'].min():.1f} km/h")
            delta_cols[2].metric("MEAN DELTA", f"{comparison['Speed Delta'].mean():.1f} km/h")
            st.caption(f"Delta = {driver} speed − {controls['driver_2']} speed, aligned on distance.")
